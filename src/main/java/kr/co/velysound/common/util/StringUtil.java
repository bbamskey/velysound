...
public class StringUtil {
	 ...
	 * </pre>
	 */
	public static String nullConvert(Object src, String returnStr) {
		//if (src != null && src.getClass().getName().equals("java.math.BigDecimal")) {
		if (src != null && src instanceof BigDecimal) {
			return ((BigDecimal) src).toString();
		}

		if (src == null || src.equals("null") || src.equals("undefined") || "".equals(src)) {
			return returnStr;
		} else {
			return ((String) src).trim();
		}
	...
	public static String nullConvert(String src) {
		...
		if (src == null || src.equals("null") || "".equals(src) || " ".equals(src) || src.equals("undefined")) {
			return "";
		} else {
			return src.trim();
		}
	 ...
	 */
	public static String[] nSplit(String source, String separator, int n) throws NullPointerException {
		String[] returnVal = null;
		if (source == null || source.equals("") || separator == null || separator.equals("") || n == 0) {
			return returnVal;
		}
		int cnt = 0;
		int idx = 0;
		String[] temp = source.split("\\" + separator);
		int size = (temp.length / n) + (temp.length % n > 0 ? 1 : 0);
		returnVal = new String[size];
		String value = "";
		for (String p : temp) {
			value += p + separator;
			if ((cnt + 1) % n == 0) {
	...
	/**
	 * 날짜형식에서 "-", "/", ":" 문자열을 제거하고 리턴한다.
	 *
	 * @param srcString 날짜형식의 문자열
	 * @return String 날짜형식에서 "-", "/", ":" 문자열을 제거하고 리턴한다.
	 * @see
	 */
	public static String getDateStr(String srcString) {
	 ...
	 *
	 * @param 문자열
	 * @return 카멜표기법으로 변환된 문자열, null이 입력되면 <code>null</code> 리턴
	 */
