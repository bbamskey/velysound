		...
		imgLiquid: function (options) {
			...
			return this.each(function ($i) {
				...
				function getSettingsOverwrite() {
					...
					if (
						imgLiquid.isIE &&
						imgLiquidRoot.settings.ieFadeInDisabled
					)
				...
				function makeOldProcess() {
					...
					if (
						...
						hn = Math.floor(
							$imgCW *
								($img.data("oheight") / $img.data("owidth"))
						);
					} else {
						w = "auto";
						h = "100%";
...
(function ($) {
	...
	$.fn.caroufredsel = $.fn.carouFredSel = function (options, configs) {
		...
		FN._bind_events = function () {
			...
			$cfs.bind(cf_e("pause", conf), function (e, imm, res) {
				...
				if (opts.auto.onTimeoutPause) {
					var dur1 = opts.auto.timeoutDuration - tmrs.timePassed,
						perc =
							100 -
							Math.ceil((dur1 * 100) / opts.auto.timeoutDuration);
			...
			$cfs.bind(cf_e("slide_prev", conf), function (e, sO, nI) {
				...
				if (!is_number(nI)) {
					if (opts.items.visibleConf.variable) {
						...
						nI = gn_getScrollItemsPrevFilter(
							a_itm,
							opts,
							itms.total - 1,
							xI
				...
				if (opts.usePadding) {
					if (i_new_l.not(i_cur_l).length) {
						a_cur[opts.d["marginRight"]] =
							i_cur_l.data("_cfs_origCssMargin");
					...
					if (i_new_l.not(i_old_l).length) {
						a_old[opts.d["marginRight"]] =
							i_old_l.data("_cfs_origCssMargin");
						_s_paddingold = function () {
							i_old_l.css(a_old);
						};
						_a_paddingold = function () {
					...
					if (pR >= 0) {
						a_new[opts.d["marginRight"]] =
							i_new_l.data("_cfs_origCssMargin") +
				...
				_moveitems = function () {
					if (overFill > 0) {
						...
						i_old = $(
							$cfs
								.children()
								.slice(
									itms.total - (opts.items.visible - overFill)
								)
								.get()
								.concat(
									$cfs.children().slice(0, overFill).get()
								)
						);
					}
					...
					if (opts.usePadding) {
						var l_itm = $cfs
							.children()
							.eq(opts.items.visible + nI - 1);
						l_itm.css(
							opts.d["marginRight"],
							l_itm.data("_cfs_origCssMargin")
						);
					}
				};
			...
			$cfs.bind(cf_e("synchronise", conf), function (e, s) {
				...
				for (var j = 0, l = s.length; j < l; j++) {
					if (
						!s[j][0].triggerHandler(cf_e("slideTo", conf), [
							n,
							s[j][3],
							true,
						])
			...
			$cfs.bind(cf_e("updateSizes", conf), function (e) {
				...
				if (!opts.circular && itms.first != 0 && vI > itms.first) {
					if (opts.items.visibleConf.variable) {
						var nI =
							gn_getVisibleItemsPrev(a_itm, opts, itms.first) -
							itms.first;
					} else if (opts.items.filter != "*") {
						var nI =
		...
		FN._bind_buttons = function () {
			...
			if (opts.prev.button) {
				opts.prev.button.bind(
					cf_e(opts.prev.event, conf, false),
					function (e) {
						e.preventDefault();
			...
			if (opts.next.button) {
				opts.next.button.bind(
					cf_e(opts.next.event, conf, false),
					function (e) {
						e.preventDefault();
	...
	function gn_getVisibleOrg($c, o) {
		return (
			o.items.visibleConf.org ||
			$c.children().slice(0, o.items.visible).filter(o.items.filter)
				.length
		);
	}
	...
	function ms_getLargestSize(i, o, dim, wrapper) {
		...
		dim =
			dim.toLowerCase().indexOf("width") > -1
				? "outerWidth"
				: "outerHeight";
	...
	function cf_sortParams(vals, typs) {
		...
		for (var a = 0, l1 = vals.length; a < l1; a++) {
			for (var b = 0, l2 = typs.length; b < l2; b++) {
				if (
					typs[b].indexOf(typeof vals[a]) > -1 &&
					is_undefined(arr[b])
				) {
					arr[b] = vals[a];
					break;
	...
	function in_complementItems(obj, opt, itm, sta) {
		...
		if (!obj[opt.d["width"]]) {
			...
			if (opt.responsive) {
				...
				obj[opt.d["width"]] = ms_getTrueLargestSize(
					itm,
					opt,
					"outerWidth"
				);
			...
			//   non-responsive -> measure it or set to "variable"
				...
				obj[opt.d["width"]] = ms_hasVariableSizes(
					itm,
					opt,
					"outerWidth"
				)
	...
	function in_complementVisibleItems(opt, avl) {
		...
		if (!opt.items.visibleConf.variable) {
			...
			//  measure and calculate primary size and visible-items
				...
				opt[opt.d["width"]] =
					opt.items.visible * opt.items[opt.d["width"]];
  ...
  */
	...
	function TouchSwipe(element, options) {
		...
		function createFingerData(index, evt) {
			...
			fingerData[index].start.x = fingerData[index].end.x =
				evt.pageX || evt.clientX;
			fingerData[index].start.y = fingerData[index].end.y =
		...
		_initonce: function (el) {
			...
			if (options.scrolllock) {
				...
				if (typeof scrollbarwidth === "undefined") {
					...
					scrollbarwidth =
						child.innerWidth() - child.height(99).innerWidth();
					parent.remove();
				}
			}
			...
			if (!$el.attr("id")) {
				$el.attr(
					"id",
					"j-popup-" + parseInt(Math.random() * 100000000)
				);
			}
			...
			$el.wrap(
				'<div id="' + el.id + '_wrapper" class="popup_wrapper" />'
			);
			...
			$(openelement).each(function (i, item) {
				...
				if (!$(item).attr("id")) {
					$(item).attr(
						"id",
						"open_" + parseInt(Math.random() * 100000000, 10)
					);
				}
			});
		...
		reposition: function (el, ordinal) {
			...
			if (options.type == "tooltip") {
				...
				if (options.triggerevent) {
					...
					$elementclicked = $(options.openelement).filter(
