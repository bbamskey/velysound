...
$(function () {
	...
	function resizeBox() {
		...
		function scrollEvent() {
			...
			//$('.wel_hero_banner .swiper-slide').css('height', winH  - locS ) ;    �ㅻ퉬二쇱뼹 �믪씠
		}
	...
	$('.breadcrumb .link').bind('click mouseover', function () {
		if ($(this).closest('.li').hasClass('act')) {
			...
			$(this)
				.closest('.li')
				.find('.dep ul')
				.css('min-width', $(this).closest('.li').find('.link').innerWidth() + 2);
		}
		return false;
	});
...
$(function () {
	...
	$('.btn_pay').click(function () {
		$('html, body')
			.stop()
			.animate(
				{scrollTop: $('.pro_info').offset().top - $('.header_cont').innerHeight() + 30},
				800,
			);
		return false;
	});
	...
	function scrollEvent2() {
		if (isMobile()) {
			var scrollEnd =
				$(document).height() - window.visualViewport.height - $('.footer_box').innerHeight() + 10;
		} else {
			var scrollEnd =
				$(document).height() - $(window).height() - $('.footer_box').innerHeight() + 10;
		}
	...
	function scroll_button_resize() {
		...
		//$('html, body').stop().animate({scrollTop:$(window).scrollTop() + 1}, 0);//22.09.18 �섏젙
		scrollEvent2();
	}
